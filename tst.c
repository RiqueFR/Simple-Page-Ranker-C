...
static char fix_letter(char letter) {
...
Tst* recursive_insert(Tst* tst, char* palavra, int i) {
    char letter = fix_letter(palavra[i]);
    if (tst == NULL) {
        tst = init_tst();
        tst->letter = letter;
    }
    if (letter < tst->letter)
        tst->left = recursive_insert(tst->left, palavra, i);
    else if (letter == tst->letter) {
        if (letter != '\0')
...
int get_word_tst(Tst* tst, char* word) {
    ...
    int i = 0;
    while (aux != NULL) {
        char letter = fix_letter(word[i]);
        if (letter < aux->letter)
            aux = aux->left;
        else if (letter == aux->letter) {
